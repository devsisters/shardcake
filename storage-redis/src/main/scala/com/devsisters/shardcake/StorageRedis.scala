...
object StorageRedis {
  ...
  type Redis        = Has[RedisCommands[Task, String, String]] with Has[PubSubCommands[fs2Stream, String, String]]

  /**
   * A layer that returns a Storage implementation using Redis
   */
  val live: ZLayer[Redis with Has[RedisConfig], Nothing, Has[Storage]] =
    (for {
      ...
      def assignmentsStream: ZStream[Any, Throwable, Map[ShardId, Option[PodAddress]]] =
        pubSubClient.subscribe(RedisChannel(config.assignmentsKey)).toZStream(1).mapM(_ => getAssignments)
